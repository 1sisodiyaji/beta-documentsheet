...
const CreateNewSheet = () => {
  ...
  const districts = docData.state ? StatesData.find((state) => state.state === docData.state)?.districts || [] : [];
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="Create a new document sheet quickly and easily. Our secure platform helps you generate, manage, and store your important documents with QR code access." />
        <meta name="keywords" content="create document, new sheet, document generation, QR code document, secure documents, digital documents, document management" />
        <meta property="og:title" content="Create New Document Sheet - Easy Document Creation" />
        <meta property="og:description" content="Start creating your new document sheet now. Our platform makes it easy to generate, manage, and securely store your important documents." />
      ...
      <motion.div className="max-w-7xl mx-auto" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
        <div className="flex md:flex-row flex-col-reverse justify-center items-center py-6 md:mb-0 mb-16 px-2 mx-auto w-full">
          <motion.div className="md:w-2/3 w-full space-y-4" initial={{ x: -50, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.2 }}>
            {/* Input fields */}
            <motion.div className="grid grid-cols-1 md:grid-cols-2 gap-6" initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.3 }}>
              <div className="flex flex-col p-4 border-2 border-gray-200 rounded-lg">
                ...
                <motion.input whileFocus={{ scale: 1.02 }} transition={{ type: 'spring', stiffness: 300 }} type="text" name="UserName" value={docData.UserName} onChange={handleInputChange} id="UserName" placeholder="Enter Name" className="py-3 px-4 border-2 border-gray-300 rounded-md focus:outline-none focus:border-green-500 ring-4 ring-green-100" />
              ...
              <div className="flex flex-col p-4 border-2 border-gray-200 rounded-lg">
                ...
                <input type="text" name="Reason" value={docData.Reason} onChange={handleInputChange} id="Reason" maxLength={300} placeholder="Enter Reason" className="py-3 px-4 border-2 border-gray-300 rounded-md focus:outline-none focus:border-green-500 ring-4 ring-green-100" />
                {errors.Reason && <p className="text-red-500 text-xs">{errors.Reason}</p>}
              </div>
            </motion.div>

            {/* Village and Post */}
            <motion.div className="grid grid-cols-1 md:grid-cols-2 gap-6" initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.4 }}>
              <div className="flex flex-col p-4 border-2 border-gray-200 rounded-lg">
                <label className="mb-2 font-medium text-gray-700" htmlFor="Village">
                  Village <span className="text-red-500">*</span>
                </label>
                <input type="text" name="Village" value={docData.Village} onChange={handleInputChange} id="Village" maxLength={30} placeholder="Enter Your Village" className="py-3 px-4 border-2 border-gray-300 rounded-md focus:outline-none focus:border-green-500 ring-4 ring-green-100" />
              ...
              <div className="flex flex-col p-4 border-2 border-gray-200 rounded-lg">
                ...
                <input type="text" name="Post" value={docData.Post} onChange={handleInputChange} id="Post" maxLength={30} placeholder="Enter Post" className="py-3 px-4 border-2 border-gray-300 rounded-md focus:outline-none focus:border-green-500 ring-4 ring-green-100" />
                {errors.Post && <p className="text-red-500 text-xs">{errors.Post}</p>}
              </div>
            </motion.div>

            {/* State, District, and Date */}
            <motion.div className="grid grid-cols-1 md:grid-cols-3 gap-6" initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.5 }}>
              <div className="flex flex-col p-4 border-2 border-gray-200 rounded-lg">
                <label className="mb-2 font-medium text-gray-700" htmlFor="state">
                  Select State
                </label>
                <select id="state" name="state" value={docData.state} onChange={handleInputChange} className="py-3 px-4 border-2 border-gray-300 rounded-md focus:outline-none focus:border-green-500 ring-4 ring-green-100">
              ...
              <div className="flex flex-col p-4 border-2 border-gray-200 rounded-lg">
                ...
                <select id="District" name="District" value={docData.District} onChange={handleInputChange} className="py-3 px-4 border-2 border-gray-300 rounded-md focus:outline-none focus:border-green-500 ring-4 ring-green-100">
            ...
            <motion.div className="flex md:flex-row flex-col gap-4" initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.6 }}>
              {/* Number of Sheets */}
              <div className="flex flex-col p-4 border-2 border-gray-200 rounded-lg w-full ">
                ...
                <motion.select whileFocus={{ scale: 1.02 }} transition={{ type: 'spring', stiffness: 300 }} id="NumberOfSheet" name="NumberOfSheet" value={docData.NumberOfSheet} onChange={handleInputChange} className="py-3 px-4 border-2 border-gray-300 rounded-md focus:outline-none focus:border-green-500 ring-4 ring-green-100">
                  {[1, 2, 3, 4, 5].map((num) => (
              ...
              <div className="flex justify-center items-center p-1">
                <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} disabled={loading} onClick={onHandleDocCreated} className={`w-80 h-12 py-3 rounded-xl focus:outline-none ring-2 transition-all duration-700 ring-green-100 bg-green-300 text-black hover:bg-green-400 ${loading ? 'cursor-not-allowed opacity-50' : ''}`}>
                  {loading ? (
          ...
          {/* Animation Section */}
          <motion.div className="hidden md:block md:w-1/3 w-full" initial={{ x: 50, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.3 }}>
